...
- Completed agent created on [01 - Create your first conversational agent in Azure Logic Apps](./01-create-first-conversational-agent.mdx) module.
...
1. In the left pane, under Development Tools, select Run history.
1. On the Run history page, in the runs table, select the latest workflow run.
1. If the Status column shows a Running status, the agent workflow is still working.

    ![Screenshot shows the run history for the workflow](media/02-debug-agent/run-history-Consumption.png)
