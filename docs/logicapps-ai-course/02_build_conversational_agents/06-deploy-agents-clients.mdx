...
- **App Service authentication (EasyAuth)**: If EasyAuth is configured on the logic app, it takes precedence over API key-based authentication. This method is typically used when OAuth flows or on-behalf-of (OBO) scenarios are required, such as interacting with connectors that require user context (explained in the extension modules - [Add user context to your tools](./04-add-user-context-to-tools.mdx)).
...
### Step 2a - Setup OAuth Authentication
This is required for the standalone chat client application.
1. Go to Authorization tab, from the left blade.
1. Check the **Agent rule** checkbox and provide the list of Object IDs to be allowed to access the agent.
![Screenshot shows Authentication blade entry point.](./media/06-deploy-agents-clients/authblade-consumption.png)

### Step 2b - Using the standalone chat client
1. Make sure your workflow is running, if not, hit run from the designer or the Run history blade.
1. Go to chat blade and follow the chat client uri to open the standalone experience.
![Screenshot shows App registration.](./media/06-deploy-agents-clients/agent-chat-consumption.png)
1. The Chat client here is using OAuth authentication you configured in Step 2a and can be used directly for production scenarios.

</TabItem>
</Tabs>
## Related content

- [Create First Conversational Agent](./01-create-first-conversational-agent.mdx)
