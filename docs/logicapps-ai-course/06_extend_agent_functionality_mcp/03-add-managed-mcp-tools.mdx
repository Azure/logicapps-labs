...
### Step 2 - Add the managed MCP Server for email 
...
1. In this case we want to use the account of the user chatting to the agent, so we would select the "Create as per-user connection?" checkbox so that the signs in happens when the user is chatting to the agent.
